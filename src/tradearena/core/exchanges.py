...
class BinanceProvider(ExchangeProvider):
    ...
    async def fetch_klines(
        self, client, symbol, interval, start_ms, end_ms
    ) -> list[list]:
        ...
        for _ in range(10):  # safety limit
            ...
            for c in data:
                ...
                if ts < start_ms:
                    continue
                all_candles.append(
                    _binance_kline(ts, c[1], c[2], c[3], c[4], c[5])
                )
...
DEFAULT_PROVIDERS: list[ExchangeProvider] = [
    BinanceProvider(),
    OKXProvider(),
    KrakenProvider(),
]
    ...
    for provider in providers:
        try:
            ...
            logger.info(
                ...
                symbol, provider.name, len(klines),
            )
            return klines
            ...
            delisted_count += 1
            logger.info(
                "Symbol %s not found on %s", symbol, provider.name
            )
            continue
        except (httpx.HTTPError, ExchangeError) as exc:
            last_error = exc
            logger.warning(
                "Exchange %s failed for %s: %s", provider.name, symbol, exc
            )
            continue

    if delisted_count == len(providers):
        raise SymbolNotFound(
            f"Symbol {symbol} not found on any exchange (likely delisted)"
        )
    if last_error:
        raise ExchangeError(
            f"All exchanges failed for {symbol}: {last_error}"
        )
    return []
        ...
        try:
            ...
            if price is not None:
                logger.info(
                    ...
                    symbol, provider.name, price,
                )
                return price
            logger.info(
                "Exchange %s returned no price for %s at %d, trying next",
                provider.name, symbol, ts_ms,
            )
        except SymbolNotFound:
            delisted_count += 1
